...
//                        } while (false && distance(origin) > point.distance(origin))
    ...
    override fun beforeDraw(drawer: Drawer, program: Program) = with(program) {
        ...
        color = color.toHSLa().run { copy(l = min(max(l + Random.double(-.04, .04), 0.1), 0.9)) }.shiftHue(Random.double(-.3, 0.4)).toRGBa()
        drawer.stroke = color
        val centerX = width / 2
...
//    }
    program {
        ...
        extend(TrisolarExtension(200, ColorRGBa.fromHex("#00FF18").toHSLa().shiftHue(Random.double(0.0,360.0)).toRGBa(), radius, travelSpeed, Point(), 5, true))
    }
}
