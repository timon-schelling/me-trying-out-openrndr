    ...
    override fun setup(program: Program): Unit = with(program) {
        GlobalScope.launch {
            val particleUnsafe = (1..100).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 1.5, ColorRGBa.BLACK, Type.HUNTER) } +
                    (1..300).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 1.5, ColorRGBa.fromHex("#910000").toHSLa().shiftHue(Random.double(0.0,360.0)).toRGBa(), Type.RUNNER) }
            while (true) {
