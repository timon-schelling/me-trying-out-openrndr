...
class Draw(override var enabled: Boolean = true) : Extension {
    ...
    override fun setup(program: Program): Unit = with(program) {
        GlobalScope.launch {
            val particleUnsafe = (1..10).map { Particle(Vector2.ZERO.randomPointInRadius(500.0), 40.0, ColorRGBa.BLUE, Type.HUNTER) } +
                    (1..10).map { Particle(Vector2.ZERO.randomPointInRadius(500.0), 40.0, ColorRGBa.RED, Type.RUNNER) }
            while (true) {
                        ...
                        launch {
                            ...
                            when (element.type) {
                                Type.RUNNER -> {
                                    ...
                                    when {
                                        alieDistance < 6.0 && !alie.teleport -> {
                                    ...
                                    element.position += ((element.position - enemy.position).normalized * 2.0) + Vector2.ZERO.randomPointInRadius(0.4)
                                }
                                Type.HUNTER -> {
                                    ...
                                    element.position -= ((element.position - enemy.position).normalized * 1.6) + Vector2.ZERO.randomPointInRadius(0.4)
                                }
                            }
                        }
                    }
