...
class Draw(override var enabled: Boolean = true) : Extension {
    ...
    override fun setup(program: Program): Unit = with(program) {
        GlobalScope.launch {
            val particleUnsafe = (1..200).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 5.0, color1, Type.HUNTER) } +
                    (1..600).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 3.0, color2, Type.RUNNER) }
            while (true) {
