...
class Draw(override var enabled: Boolean = true) : Extension {
    ...
    override fun setup(program: Program): Unit = with(program) {
        GlobalScope.launch {
            val particleUnsafe = (1..60).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 1.5, ColorRGBa.BLACK, Type.HUNTER) } +
                    (1..200).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 1.5, ColorRGBa.fromHex("#910000"), Type.RUNNER) }
            while (true) {
