...
class Draw(override var enabled: Boolean = true) : Extension {
    ...
    override fun setup(program: Program): Unit = with(program) {
        GlobalScope.launch {
            val particleUnsafe = (1..100).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 5.0, ColorRGBa.WHITE_SMOKE, Type.HUNTER) } +
                    (1..300).map { Particle(Vector2.ZERO.randomPointInRadius(radius), 3.0, ColorRGBa.fromHex("#000EAE").toHSLa().shiftHue(Random.double(-20.0,20.0)).toRGBa(), Type.RUNNER) }
            while (true) {
