...
class Draw(override var enabled: Boolean = true) : Extension {
    ...
    override fun setup(program: Program): Unit = with(program) {
        GlobalScope.launch {
            val particleUnsafe = (1..100).map { Particle(Vector2.ZERO.randomPointInRadius(300.0), 40.0, ColorRGBa.BLUE, Type.HUNTER) } +
                    (1..100).map { Particle(Vector2.ZERO.randomPointInRadius(300.0), 40.0, ColorRGBa.RED, Type.RUNNER) }
            while (true) {
                        ...
                        launch {
                            when (element.type) {
                                Type.RUNNER -> {
                                    ...
                                    element.position += ((element.position - enemy.position).normalized * 2.0) + Vector2.ZERO.randomPointInRadius(0.3)
                                }
                                Type.HUNTER -> {
                                    ...
                                    element.position -= ((element.position - target.position).normalized * 2.0) + Vector2.ZERO.randomPointInRadius(0.3)
                                }
                            }
                        }
                    }
